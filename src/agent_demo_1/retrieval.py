...
_RERANKER_CACHE: tuple[Any, Any, Any] | None = None
...
def retrieve_chunks(
    ...
    *,
    ...
    with persisting._locked_store():
        index, manifest = _load_snapshot_or_empty()
        rows = _read_chunks()
    ...
    if len(scores) != len(candidates):
        raise RuntimeError(
            f"reranker returned {len(scores)} scores; expected {len(candidates)}"
        )
...
def _load_snapshot_or_empty() -> tuple[Any | None, Mapping[str, Any]]:
    try:
        return persisting.load_index()
    except FileNotFoundError:
        index_path = persisting.DATA_DIRECTORY / persisting.FAISS_FILENAME
        manifest_path = persisting.DATA_DIRECTORY / persisting.MANIFEST_FILENAME
...
def _load_reranker() -> tuple[Any, Any, Any]:
    """Load the cross-encoder once, with an explicit single-flight cache."""
    ...
    with _RERANKER_LOCK:
        ...
        model.to(device)
        model.eval()
        _RERANKER_CACHE = tokenizer, model, device
        return _RERANKER_CACHE
...
def _rerank(query: str, passages: Sequence[str]) -> list[float]:
    ...
    # Hold the same lock across cache lookup and inference.  Besides making
    # initialisation single-flight, this avoids concurrent calls mutating or
    # racing a shared torch module on CUDA/MPS/CPU backends.
